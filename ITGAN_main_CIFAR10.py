...
def main():
    ...
    parser.add_argument('--batch_train_z', type=int, default=100, help='batch size for sampling z, it should be a divisor of the image number per class per split')
    ...
    for itz in range(args.Iteration+1):
        ...
        for c in range(num_classes):
            ...
            if args.z_sample_mode == 'fixed': # fixed combination of mini-batches
                idx_rand = np.arange(num_imgpercls)
            elif args.z_sample_mode == 'random': # random combination of mini-batches
                idx_rand = np.arange(num_imgpercls)
                np.random.shuffle(idx_rand)
            else:
                idx_rand = None
                exit('unknown args.z_sample_mode: %s'%args.z_sample_mode)
            ...
            for bc in range(int(np.ceil(num_imgpercls//args.batch_train_z))): # args.batch_train_z should be a divisor of num_imgpercls
                ...
                # sample a small batch of z_c and generate images
                idx = idx_rand[bc * args.batch_train_z: (bc + 1) * args.batch_train_z]
                z_syn = z_split[c][idx]
